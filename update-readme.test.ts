...
describe('test suites for update-readme', () => {
  describe('test suites for filePattern', () => {
    test('should return true for a valid file name', () => {
      expect(filePattern.test('observed-pin.ts')).toBe(true);
    });

    test('should return true for a valid file name containing digits', () => {
      expect(filePattern.test('hello-world-123.ts')).toBe(false);
    });

    test('should return true for a file name containing a test extension', () => {
      expect(filePattern.test('observed-pin.test.ts')).toBe(true);
    });
  ...
  describe('test suites for getKatasByKyu', () => {
    test('should return the length of 2 for the valid file names', async () => {
      ...
      const fileNames = ['hello-world.ts', 'hello-world.data.ts', 'hello-world.test.ts', 'hello-world-123.ts', 'hello-world-123.test.ts', 'observed-pin.ts', 'observed-pin.test.ts'];
